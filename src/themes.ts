...
export interface ThemeDefinition {
  ...
  colors: Omit<
    ThemeColors,
    "status" | "primaryAccent" | "primaryAccentHover" | "link"
  > & {
    primaryAccent?: string;
    primaryAccentHover?: string;
    link?: string;
  };
  status?: Partial<Record<StatusName, Partial<StatusColors>>>;
...
function createAntDesignComponentOverrides({
  ...
  colors: Omit<ThemeColors, "status">;
  controlHeight: number;
  radius: ThemeDefinition["radius"];
  shadow: string;
  status: Record<StatusName, StatusColors>;
...
function originalAdaptation(source: ThemeDefinition["source"]) {
...
export function createTheme(definition: ThemeDefinition): Theme {
  ...
  const status = Object.fromEntries(
    Object.entries(defaultStatus).map(([key, defaults]) => {
      const values = {
        ...defaults,
        ...definition.status?.[key as StatusName],
      };
      return [key, { ...values, border: values.border ?? values.background }];
    }),
  ) as Record<StatusName, StatusColors>;
...
const REQUIRED_COLORS = [
  ...
  "selectionText",
  "codeBackground",
] as const satisfies readonly (keyof Omit<ThemeColors, "status">)[];
...
export function validateThemeCatalog(themes: readonly Theme[] = THEMES) {
  ...
  for (const theme of themes) {
    ...
    if (
      ...
      typeof antDesign.cssVar !== "object" ||
      !antDesign.token ||
    ...
    for (const field of ["sans", "heading", "mono"] as const) {
...
export function renderThemeCss(theme: Theme) {
